...
**
** $Header: /cvsroot/glide/glide2x/h3/glide/src/digutex.c,v 1.1.1.1 1999/12/07 21:49:26 joseph Exp $
** $Log: digutex.c,v $
                    ...
                    ))
{
  ...
  GR_BEGIN_NOFIFOCHECK_RET("guTexAllocateMemory",99);
                      ...
                      ))
{
  ...
  GR_BEGIN_NOFIFOCHECK_RET("guTexChangeAttributes",88);
...
{
  GR_BEGIN_NOFIFOCHECK_RET("guTexGetCurrentMipMap",99);
...
{
  GR_BEGIN_NOFIFOCHECK_RET("guTexGetMipMapInfo",99);
...
{
  GR_BEGIN_NOFIFOCHECK_RET("guTexMemQueryAvail",99);
  GDBG_INFO_MORE(gc->myLevel,"(%d)\n",tmu);
  GR_CHECK_TMU( myName, tmu );
